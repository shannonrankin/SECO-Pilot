...
# Environmental Data
* add environmental data to birdnet_all and birdnetBins_all using PAMverse
```{r env-data}
test <- birdnet_All %>%
  rename(UTC = datetime_UTC)
test <- matchGFS(test)
...
birdnet_top10_TJE <- birdnet_All %>% 
  ...
  slice_head(n = 10) %>% # Get top 10 species
  left_join(birdnet_All, by = "Common.Name") # join back to original data
write_csv(birdnet_top10_TJE, here("output", "birdnet_top10_TJE.csv"))
...
birdnet_top10_SCI <- birdnet_All %>% 
  ...
  slice_head(n = 10) %>% # Get top 10 species
  left_join(birdnet_All, by = "Common.Name") # join back to original data
write_csv(birdnet_top10_SCI, here("output", "birdnet_top10_SCI.csv"))
...
write_csv(birdnet_LFRR_DD, here("output", "birdnet_LFRR_DD.csv"))
...
write_csv(birdnet_SCI, here("output", "birdnet_SCI.csv"))

birdnet_SPP <- filter(birdnet_All, Location == c("SPP", "SGR"))
write_csv(birdnet_SPP, here("output", "birdnet_SPP.csv"))

...
NEXT: Need to modify existing selection table to change ALL values of low frequency to 2200 Hz and ALL values of high frequency to 4600 Hz. Then rerun process_birdnet_table. 

```{r birdnet_modify-in-place}
# Root path to Birdnet data folders
root_path <- "E:/Rails/recordings/SPP-3S/SPP-3S_48s+s_20250515/"
...
#add deployment_ID column and modify high/low frequency (for measurements)
add_deploymentID(root_path, lowFreq = 2200, hiFreq = 5000)


#Copy path from the add_deploymentID console output:
selection_table_path <-  "E:/Rails/recordings/SPP-3S/SPP-3S_48s+s_20250515/birdnetLocal_SPP-3S_48s+s_20250515/BirdNET_SelectionTable_SPP-3S_48s+s_20250515.txt"
...
If your next step is to validate data in Raven, go to: /_code/02_ravenValidate.Rmd
Or else, go to /_code/03_ingestData.Rmd

...
# Big Data Paths
bigDataPath <- "bigData/rails"
bigDataRawFiles <- "bigData/rails/rawBirdnetFiles"

# Destination path to move Birdnet files TO:
dest_path <- bigDataRawFiles
...
birdnet_all <- birdnet_add_datetime(data, tz = "UTC") %>% #add UTC dateTime columns
  ...
  left_join(deployments_all, by = "deployment_id") %>%
...
write_csv(birdnet_all, file= here(paste(bigDataRawFiles, birdnetFileName,".csv", sep="")))
...
birdnet_bins_day <- binDetectionData(x = birdnet_day, bin = "day", columns = c("common_name", "deployment_id"), rematchGPS = TRUE, gpsGroup = "deployment_id") %>%
  left_join(deployments_select, by = "deployment_id")%>%
  rename(Latitude = latitude, Longitude = longitude)
birdnet_bins_day_env <- matchGFS(birdnet_bins_day)

saveRDS(birdnet_bins_day_env, file = here(paste("data/", birdnetFileName, "_bins_day.rds")))
...
saveRDS(birdnet_bins_hour_env, file = here(paste("data/", birdnetFileName, "_bins_hour.rds")))
```

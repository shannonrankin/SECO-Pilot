...
deployTable <- as_tibble(readRDS(here("data/deployDetails.rds"))) %>%
...
Birdnet Analyzer v2.4 was used to batch analyze recordings for each deployment, with detections were saved as csv files and formatted as Raven selection tables. Preliminary data were analyzed using R programming language and preliminary results are provided to assess the potential use of PAM for studying Ridgway's Rails. Preliminary data products and R code are made publicly available in this repository, however some data products exceeded the storage limitations. These data products can be reproduced from the preliminary data products using code saved within the repository.
